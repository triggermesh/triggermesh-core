...
// controller reconciling v1alpha1.RedisBroker.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.RedisBroker. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.RedisBroker) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.RedisBroker.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.RedisBroker. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.RedisBroker) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.RedisBroker if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.RedisBroker.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.RedisBroker) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.RedisBroker) reconciler.Event
...
type reconcilerImpl struct {
	...
	Lister eventingv1alpha1.RedisBrokerLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client internalclientset.Interface, lister eventingv1alpha1.RedisBrokerLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.RedisBrokers(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.RedisBroker, desired *v1alpha1.RedisBroker) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.EventingV1alpha1().RedisBrokers(desired.Namespace)
		...
		updater := r.Client.EventingV1alpha1().RedisBrokers(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.RedisBroker, desiredFinalizers sets.String) (*v1alpha1.RedisBroker, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.EventingV1alpha1().RedisBrokers(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.RedisBroker) (*v1alpha1.RedisBroker, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.RedisBroker, reconcileEvent reconciler.Event) (*v1alpha1.RedisBroker, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
